...
seL4_Word 
page_entry_number(seL4_Word page)
{
...
void 
page_update_entry(seL4_Word *page_entry, uint8_t bits, seL4_Word num)
{
    *page_entry = num;
...
int 
page_table_insert(struct page_table *page_table, seL4_Word vaddr, seL4_Word page_num) 
{
...
seL4_Word * 
page_lookup(struct page_table *page_table, seL4_Word vaddr)
{
    struct pgd *pgd = page_table->pgd;
    struct pt_index ind = get_pt_index(vaddr);
    struct pud *pud = pgd->pud[ind.l1];
...
int 
page_table_remove(struct page_table* page_table, seL4_Word vaddr) 
{
...
void 
page_table_destroy(struct page_table * page_table, cspace_t * cspace)
{
    struct pgd *pgd = page_table->pgd;
    for (unsigned int i = 0; i < PAGE_INDEX_SIZE; i++) {
        if (pgd->pud[i] != NULL) {
...
void 
save_seL4_info(struct page_table * page_table, ut_t * ut, seL4_CPtr slot)
{
    struct seL4_page_objects_frame * frame;
    if (page_table->seL4_pages == NULL) {
        ...
        if (frame->size == PAGE_OBJECT_SIZE) {
